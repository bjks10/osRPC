...
%% load NBDPS data %%
 load(strcat('sim_osRPCdataA',num2str(sim_n),'.mat'))
...
%% AUGMENTATION FOR ORDERED PROBIT %%
del1=norminv(1/3);
del2=norminv(2/3);


s0=50; %large scale to cover wide range
...
for k=1:k_max
   ...
   w_ip(:,k)=ones(n,1);
   W_temp=[w_sid w_ip];
   Wxi_temp=W_temp*transpose(xi_iter);
   phi_temp1=normcdf((del1-Wxi_temp)/s0);
   phi_temp2=normcdf((del2-Wxi_temp)/s0);
   py1=phi_temp1;
   py2=phi_temp2-phi_temp1;
   py3=1-phi_temp2;
   probit_kp=(y_cc==1).*log(py1)+(y_cc==2).*log(py2)+(y_cc==3).*log(py3);
...
WXi_iter=Wup*transpose(xi_iter);
%truncation for case 1 (-inf,gamma1)
z_probit(y_cc==1)=truncnorm2(WXi_iter(y_cc==1),1,-inf,del1);
%truncation for case 2 (gamma1,gamma2)
z_probit(y_cc==2)=truncnorm2(del1+WXi_iter(y_cc==2),1,del1,del2);
%truncation for case 3 (gamma2, inf)
z_probit(y_cc==3)=truncnorm2(del2+WXi_iter(y_cc==3),1,del2,inf);
   ...
   %control extremes;
    ...
    %% AUGMENTATION update %%
%truncated beta boundaries for del1
c11=max(z_probit(y_cc==1));
c12=min(z_probit(y_cc==2));
d1l=normcdf(c11/s0)/normcdf(del2/s0);
d1u=normcdf(c12/s0)/normcdf(del2/s0);
u1=rand;
d1=d1l+(d1u-d1l)*u1;
del1=norminv(normcdf(del2/s0)*d1);

%truncated beta boundaries for del2
c21=max(z_probit(y_cc==2));
c22=min(z_probit(y_cc==3));
d2l=(normcdf(c21/s0)-normcdf(del1/s0))/(1-normcdf(del1/s0));
d2u=(normcdf(c22/s0)-normcdf(del1/s0))/(1-normcdf(del1/s0));
u2=rand;
d2=d2l+(d2u-d2l)*u2;
del2=norminv(normcdf(del1/s0)+(d2*(1-normcdf(del1/s0))));
...
for ii=1:m_perm
    ...
    del_thin1=del_burn(i_thin,1);
    del_thin2=del_burn(i_thin,2);
    for s=1:S
   ...
   for k=1:kia
        ...
        w_ip(:,k)=ones(n,1);
        W_temp=[w_sid w_ip];
        Wxi_temp=W_temp*transpose(xi_itr);
        phi_temp1=normcdf((del1-Wxi_temp)/s0);
        phi_temp2=normcdf((del2-Wxi_temp)/s0);
        py1=phi_temp1;
        py2=phi_temp2-phi_temp1;
...
del_med1=median(del_burn(:,1));
del_med2=median(del_burn(:,2));
...
ep_kp=zeros(n,kia);
 for k=1:kia
        ...
        w_ip(:,k)=ones(n,1);
        W_temp=[w_sid w_ip];
        Wxi_temp=W_temp*transpose(xi_med);
        phi_temp1=normcdf((del1-Wxi_temp)/s0);
        phi_temp2=normcdf((del2-Wxi_temp)/s0);
        py1=phi_temp1;
        py2=phi_temp2-phi_temp1;
...
save(strcat('osrpc_simResults_A',num2str(sim_n)),'py_pred','pred_ci','pred_nu','DIC','DIC_star','t_I0','y_mse','nu_mse');
